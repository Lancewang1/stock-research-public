(() => {
  ...
  function renderResearch(event) {
    ...
    documents.innerHTML = researchCase.viewChanges ? researchCase.viewChanges.map(house => `<article class="house-change-card ${esc(house.key)}"><header class="house-change-header"><span class="institution-mark ${house.key === "gs" ? "northstar" : "harbor"}">${esc(house.mark)}</span><div><strong>${esc(house.institution)}</strong><small>${esc(house.vintage)}</small></div><span class="change-state ${esc(house.stateTone)}">${esc(house.state)}</span></header><section class="estimate-section"><span class="change-label">ESTIMATE PATH</span><div class="estimate-path">${house.estimates.map((item, index) => `${index ? '<i data-lucide="arrow-right"></i>' : ""}<div class="estimate-node ${esc(item.tone || "base")}"><span>${esc(item.label)}</span><strong>${esc(item.value)}</strong><small>${esc(item.note)}</small></div>`).join("")}</div></section><section class="change-section"><span class="change-label">FOMC CALL</span><div class="policy-route"><div><small>Before CPI</small><span>${esc(house.policyBefore)}</span></div><i data-lucide="arrow-right"></i><div class="after"><small>After CPI</small><strong>${esc(house.policyAfter)}</strong></div></div><p>${esc(house.policyNote)}</p></section><section class="view-section-block"><span class="change-label">ANALYST VIEW</span><div class="view-stage"><small>BEFORE</small><p>${esc(house.viewBefore)}</p></div><div class="view-shift-arrow"><i data-lucide="arrow-down"></i><span>POST-RELEASE</span></div><div class="view-stage after"><small>AFTER</small><p>${esc(house.viewAfter)}</p></div></section><div class="house-delta"><i data-lucide="scan-line"></i><span><small>WHAT CHANGED</small><strong>${esc(house.delta)}</strong></span></div><footer><i data-lucide="file-text"></i><span>${esc(house.sources)}</span></footer></article>`).join("") : researchCase.documents.map(doc => `<article><header><span class="institution-mark ${doc.mark === "GS" ? "northstar" : "harbor"}">${doc.mark}</span><div><strong>${esc(doc.institution)}</strong><small>${esc(doc.date)}</small></div><span class="stance-tag ${doc.tone}">${esc(doc.stance)}</span></header><p>${esc(doc.text)}</p><footer><i data-lucide="file-text"></i><span class="source-page">${esc(doc.ref)}</span></footer></article>`).join("");
