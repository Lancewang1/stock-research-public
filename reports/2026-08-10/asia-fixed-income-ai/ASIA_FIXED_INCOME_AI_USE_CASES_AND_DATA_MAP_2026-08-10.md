...
日期：2026-08-10  
范围：Asia Fixed Income，覆盖亚洲离岸信用债、主要本币债券、利率衍生品与相关外汇市场  
...
1. **Asia Credit Copilot**：读懂主体、债券条款、事件与组合暴露，回答“信用发生了什么变化，我应关注哪只券”。
...
### C1. 主体信用“变化检测”与滚动信用备忘录（P0）

- **谁**：买方信用分析师、行业研究员、信用 PM。
- **什么时候**：财报、经营数据、评级行动、重大公告发布后；月度/季度例行复核前。
- **要解决什么**：不是重写一份静态公司介绍，而是快速回答“与上次判断相比，偿债能力、流动性、资本结构和管理层行为究竟变了什么”。
- **现在怎么解决**：分析师下载多期财报和公告，在 Excel 更新指标，再翻卖方报告、评级报告和电话会记录；跨中文、英文、日文、韩文材料时通常还要人工翻译。一次完整更新常需数小时。
- **AI 应如何介入**：自动做多期财务桥接、识别一次性项目和会计口径变化；对管理层表述做逐期 diff；把“事实—分析—结论”分开，并对每项事实给出页码级引用；输出 bull/base/bear 三种信用路径。
- **关键数据**：审计财报、临时公告、业绩会材料、评级报告、债务到期表、债券价格/利差、同业指标、历史版信用备忘录。
- **成功指标**：首稿用时下降 60% 以上；关键数字可追溯率接近 100%；分析师对“重要变化遗漏率”的抽检持续下降。
...
### C3. 信用事件雷达与“事件到持仓”告警（P0/P1）

- **谁**：信用分析师、PM、交易员、风险经理、CIO 值班人员。
- **什么时候**：评级下调、盈利预警、监管处罚、资产出售、再融资失败、管理层变化、法律诉讼或价格跳空发生时。
- **要解决什么**：判断事件是否真正影响偿债能力、哪些债券和账户受影响、需要立即交易还是等待更多证据，并识别同一集团或行业的二阶传导。
- **现在怎么解决**：依赖新闻终端、评级邮件、微信群/聊天和价格告警；人工去重、关联发行主体与担保人，再手工查组合持仓。噪声大且夜间容易漏报。
- **AI 应如何介入**：多语言事件聚类与去重；把事件映射到法人、母公司、担保人和具体 ISIN；按“现金流、融资、契约、治理、监管、市场技术面”打影响标签；只在达到阈值时发送带证据的分级告警。
- **关键数据**：官方公告、评级行动、可信新闻、实时/延迟价格、公司关系图、债券主数据、内部持仓和预设风险规则。
- **成功指标**：高影响事件召回率、告警精确率、从首个可信来源到持仓告警的延迟、夜间人工值班负担。
...
### C6. 组合信用风险、降级/违约与流动性压力测试（P1）

- **谁**：信用 PM、独立风险、CIO、保险资管 ALM 团队。
- **什么时候**：每日风险检查、周会、评级迁移、赎回压力或宏观冲击情景下。
- **要解决什么**：看清集团、行业、国家、担保链、再融资窗口和流动性集中度；估算降级、违约、回收率变化和被迫出售对组合的影响。
- **现在怎么解决**：风险系统计算基础敏感度，分析师用 Excel 补充主体映射和情景；对“同一风险通过不同 SPV/担保人重复出现”的识别往往不完整。
- **AI 应如何介入**：统一 ultimate parent、issuer、guarantor 和 security；把自然语言情景转换为经批准的确定性压力参数；解释 P&L 来源，列出最脆弱持仓和数据缺口；禁止模型自行修改估值方法。
- **关键数据**：完整持仓、benchmark、现金流、评级迁移矩阵、PD/LGD、价格和流动性指标、集团关系、基金申赎/保险负债信息。
- **成功指标**：风险归因覆盖率、法人映射错误率、情景生成时间、与正式风险引擎结果的一致性。

### C7. 困境债、交换要约与重组回收分析（P1/P2）

- **谁**：distressed/special situations 团队、信用 PM、法律顾问、风险委员会。
- **什么时候**：利息或本金延付、交叉违约、同意征求、交换要约、破产/清盘程序启动时。
- **要解决什么**：重建完整资本结构和债权优先顺序；理解表决门槛和不同方案；估算抵押物、企业价值、时间成本和司法辖区差异下的回收率。
- **现在怎么解决**：律师、财务顾问和分析师分别维护文档与模型，人工拼接法院材料、债券条款、资产估值和债权人委员会消息。
- **AI 应如何介入**：建立可追溯事件时间线、claim tree 和 waterfall 输入表；对不同企业价值/折价率/处置期做敏感性；标记事实、传闻和法律意见的证据等级；绝不把生成文本当正式法律意见。
- **关键数据**：所有债务文件、法院/监管材料、交换条款、资产清单和估值、持有人披露、历史重组案例、内部法律意见。
- **成功指标**：资本结构遗漏率、版本同步速度、情景复算一致性、外部顾问复核效率。

### C8. 主权—银行—企业信用传导图（P1）

- **谁**：亚洲信用 PM、主权分析师、跨资产风险团队。
- **什么时候**：主权评级变化、汇率贬值、地产/银行压力、资本管制、制裁或大宗商品冲击时。
- **要解决什么**：判断风险如何从主权资产负债表传到银行融资、国企支持、企业外币债务和离岸美元债，而不是只看单一主体。
- **现在怎么解决**：各分析师按行业写孤立报告，靠会议口头拼接；政府持股、银行授信、担保、供应链和外币错配缺少统一实体图。
- **AI 应如何介入**：构建带时间版本的所有权、担保、融资、收入和政策支持知识图；根据冲击类型给出一阶/二阶暴露；同时展示反证和传导链中最不确定的节点。
- **关键数据**：主权宏观与财政、银行资产负债表、公司债务币种、政府持股、担保/授信、跨境资金流、CDS/债券/FX 市场数据。
- **成功指标**：隐藏关联风险发现数、事件后解释速度、跨团队重复劳动下降、错误关联率。
...
| BIS Data Portal | 政策利率、有效汇率、信贷、跨境银行、债务证券、衍生品统计 | A | C8、R2、F4、F6 | 频率偏低、修订和口径需要 point-in-time 管理 |
| IMF Data / IFS / WEO / COFER | 宏观、国际收支、外储、预测与全球比较 | A/B | C8、R2、F4、F6 | 部分详细数据/API 或下载方式会变化；COFER 多为聚合 |
| World Bank Data / International Debt Statistics | 宏观、外债、发展与主权指标 | A | C8、F8 | 频率低，不适合实时交易 |
...
| UN / OFAC / EU / UK HMT 等制裁清单 | 制裁主体、船舶、项目与规则 | A/B | C3、C8、F6 | 名称匹配误报高；需本地监管和商业 KYC 数据补充 |
...
1. **C1 主体信用变化 memo**：最能体现多语言长文档和跨期比较能力。
...
| 完整性 | 重要条款/事件/风险遗漏率、实体映射覆盖率 | C1/C3/C4/C7/C8 |
| 时效性 | 来源发布到结构化、告警或首稿的延迟 | C2/C3/C9/R1/R2/R3/F1/F2 |
| 计算正确性 | 与前台/风险/独立 benchmark 的差异 | C5/C6/R4/R6/R7/F3/F5 |
